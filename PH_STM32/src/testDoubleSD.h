...
/*
 SD card example - Copy binaries - Read a binary file1 and Write it into file2 at the same time
 Buffer of 8192 bytes is used for writing
 You have to place a binary called test_in.bin on the sdcard and run the sketch
 You will get test_out.bin after the copy
...
void setup()
{
    ...
    Serial1.println("Reading and Writing..");
    uint8 buf[8192];
    int16 nr;
    uint32 t = millis();
    while ((nr = myFileIn.read(buf, 8192)) > 0)
    {
        if (nr < 8192)
        {
